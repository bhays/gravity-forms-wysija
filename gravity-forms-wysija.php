...
class GFWysija {
    ...
    public static function tooltips($tooltips){
        $wysija_tooltips = array(
            "wysija_contact_list" => "<h6>" . __("MailPoet Lists", "gravity-forms-wysija") . "</h6>" . __("Select the MailPoet lists you would like to add your contacts to.", "gravity-forms-wysija"),
            "wysija_gravity_form" => "<h6>" . __("Gravity Form", "gravity-forms-wysija") . "</h6>" . __("Select the Gravity Form you would like to integrate with MailPoet. Contacts generated by this form will be automatically added the selected MailPoet lists.", "gravity-forms-wysija"),
            "wysija_welcome" => "<h6>" . __("Send Welcome Email", "gravity-forms-wysija") . "</h6>" . __("When this option is enabled, users will receive an automatic welcome email from MailPoet upon being added to your MailPoet list.", "gravity-forms-wysija"),
            "wysija_map_fields" => "<h6>" . __("Map Fields", "gravity-forms-wysija") . "</h6>" . __("Associate your MailPoet newsletter questions to the appropriate Gravity Form fields by selecting.", "gravity-forms-wysija"),
            "wysija_optin_condition" => "<h6>" . __("Opt-In Condition", "gravity-forms-wysija") . "</h6>" . __("When the opt-in condition is enabled, form submissions will only be exported to MailPoet when the condition is met. When disabled all form submissions will be exported.", "gravity-forms-wysija"),
            "wysija_double_optin" => "<h6>" . __("Double Opt-In", "gravity-forms-wysija") . "</h6>" . __("When the double opt-in option is enabled, MailPoet will send a confirmation email to the user and will only add them to your MailPoet list upon confirmation.", "gravity-forms-wysija")
        );
        return array_merge($tooltips, $wysija_tooltips);
    }

    //Creates MailPoet left nav menu under Forms
    public static function create_menu($menus){

        // Adding submenu if user has access
        $permission = self::has_access("gravityforms_wysija");
        if(!empty($permission))
            $menus[] = array("name" => "gf_wysija", "label" => __("MailPoet", "gravity-forms-wysija"), "callback" =>  array("GFWysija", "wysija_page"), "permission" => $permission);
    ...
    private static function list_page(){
        if(!self::is_gravityforms_supported()){
            die(__(sprintf("MailPoet Add-On requires Gravity Forms %s. Upgrade automatically on the %sPlugin page%s.", self::$min_gravityforms_version, "<a href='plugins.php'>", "</a>"), "gravity-forms-wysija"));
        }
        ...
        <div class="wrap">
            ...
            <h2><?php _e("MailPoet Newsletter Feeds", "gravity-forms-wysija"); ?>
                ...
                <table class="widefat fixed" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="manage-column"><?php _e("MailPoet Lists", "gravity-forms-wysija") ?></th>
                        </tr>
                    </thead>
                        ...
                        if(is_array($settings) && sizeof($settings) > 0){
                            ...
                            <tr>
                                <td colspan="4" style="padding:20px;">
                                    <?php _e(sprintf("You don't have any MailPoet Newsletter feeds configured. Let's go %screate one%s!", '<a href="admin.php?page=gf_wysija&view=edit&id=0">', "</a>"), "gravity-forms-wysija"); ?>
                                </td>
                            </tr>
                            <?php
                        }
    ...
    private static function edit_page(){
        ...
        <div class="wrap">
            <h2><?php _e("MailPoet Newsletter Feed", "gravity-forms-wysija") ?></h2>
        <?php
            ...
            <div id="wysija_field_group" valign="top" <?php echo empty($config["meta"]["lists"]) || empty($config["form_id"]) ? "style='display:none;'" : "" ?>>
                <div id="wysija_field_container" valign="top" class="margin_vertical_10" >
                    ...
                    if(!empty($config["form_id"])){
                        ...
                        //getting list of all MailPoet details for the selected newsletter
                    	$details = self::get_wysija_details();
	            ...
	            <div class="margin_vertical_10">
	                <label for="gf_wysija_list" class="left_header"><?php _e("MailPoet Lists", "gravity-forms-wysija"); ?> <?php gform_tooltip("wysija_contact_list") ?></label>
	                <?php

	                //global wysija settings
	                $settings = get_option("gf_wysija_settings");

	                //getting all MailPoet Newsletters
	                $lists = self::get_wysija_lists();

	                if (!$lists):
	                    echo __("Could not load MailPoet lists.", "gravity-forms-wysija");
                                ...
                                <td>
                                    <div id="wysija_optin_condition_field_container" <?php echo !rgar($config["meta"],"optin_enabled") ? "style='display:none'" : ""?>>
                                        <div id="wysija_optin_condition_fields" style="display:none">
                                            <?php _e("Export to MailPoet if ", "gravity-forms-wysija") ?>
    ...
    public static function select_wysija_form(){
        ...
        //getting list of all MailPoet details for the selected contact list
        $details = self::get_wysija_details();
    ...
    private static function get_field_mapping($config, $form_id, $details){
        ...
        $str = "<table cellpadding='0' cellspacing='0'><tr><td class='wysija_col_heading'>" . __("MailPoet Fields", "gravity-forms-wysija") . "</td><td class='wysija_col_heading'>" . __("Form Fields", "gravity-forms-wysija") . "</td></tr>";
    ...
    public static function export($entry, $form, $is_fulfilled = false){
        ...
        foreach($feeds as $feed){
            ...
            if(self::is_optin($form, $feed, $entry))
            {
                ...
                //updating meta to indicate this entry has already been subscribed to MailPoet. This will be used to prevent duplicate subscriptions.
	...
	public static function plugin_row()
	{
		if ( ! self::is_gravityforms_supported() || ! self::is_wysija_installed() )
		{
			...
			$message .= '<br/>'.sprintf( __( 'MailPoet Newsletters plugin is required for this to work. %sDownload it now.%s', 'gravity-forms-wysija' ), '<a href="http://wordpress.org/extend/plugins/wysija-newsletters/">', '</a>' );
    ...
    public static function uninstall(){
        ...
        if(!GFWysija::has_access("gravityforms_wysija_uninstall"))
            die(__("You don't have adequate permission to uninstall MailPoet Add-On.", "gravity-forms-wysija"));
	...
	// Clean strings from MailPoet, we don't need any HTML or line breaks
